...
const LiveStrategyStatistics = ({ opened, onClose, statistics }) => {
  ...
  <Table.Tbody>
    ...
    <Table.Tr>
      ...
      <Table striped highlightOnHover withTableBorder>
        ...
        <Table.Tbody>
          {statistics?.datewise_pnl?.map((item, index) => {
            const pnl = Number(item.cum_pnl);

            return (
              <Table.Tr key={index}>
                <Table.Td>
                  {dayjs(item.trade_date).format("DD MMM YYYY")}
                </Table.Td>
...
export default LiveStrategyStatistics;